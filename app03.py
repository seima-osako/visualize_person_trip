...
input_dir = Path("data/demo03/input")
output_dir = Path("data/demo03/output")
solver_timeout = 60
...
capacity_01 = st.sidebar.slider(
    "capacity_01", min_value=10, max_value=50, step=1, value=31
)
st.sidebar.write("### 上信観光：車両ごとの乗車定員数")
capacity_23 = st.sidebar.slider(
    "capacity_23", min_value=10, max_value=50, step=1, value=30
)
...
cost_01 = st.sidebar.slider("cost_01", min_value=100, max_value=500, step=10, value=310)
st.sidebar.write("### 上信観光：キロ単価")
cost_23 = st.sidebar.slider("cost_23", min_value=100, max_value=500, step=10, value=270)
...
dfs_common_operation = pd.DataFrame(
    data={
        ...
        "route_name": ["川曲線", "川曲線", "京目線", "京目線"],
        "route_length": [9, 9, 7, 7],
        "fare": [220, 220, 220, 220],
    }
)
